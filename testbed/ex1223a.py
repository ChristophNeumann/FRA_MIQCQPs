#  MINLP written by GAMS Convert at 01/04/19 10:28:31
#  
#  Equation counts
#      Total        E        G        L        N        X        C        B
#         10        1        0        9        0        0        0        0
...
m.obj = Objective(expr=(-1 + m.x1)**2 + (-2 + m.x2)**2 + (-3 + m.x3)**2 - m.b4 - 3*m.b5 - m.b6 - 0.693147180559945*m.b7
                        + 6, sense=minimize)

m.c1 = Constraint(expr=   m.x1 + m.x2 + m.x3 + m.b4 + m.b5 + m.b6 <= 5)

m.c2 = Constraint(expr=m.x1**2 + m.x2**2 + m.x3**2 + m.b6 <= 5.5)
...
m.c7 = Constraint(expr=m.x2**2 + m.b5 <= 1.64)

m.c8 = Constraint(expr=m.x3**2 + m.b6 <= 4.25)

m.c9 = Constraint(expr=m.x3**2 + m.b5 <= 4.64)
